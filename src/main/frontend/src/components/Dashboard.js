...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const isoStart = startDate.toISOString().slice(0, 16); // "YYYY-MM-DDTHH:mm"
  ...
  const fetchFilteredData = () => {
    const isoStart = startDate.toISOString().slice(0, 16);
    const isoEnd = endDate.toISOString().slice(0, 16);
